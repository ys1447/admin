...
@section('content')
    <h1 class="text-3xl font-semibold">Article</h1>
    <h2 class="text-lg font-light"> All articles in the database </h2>
                ...
                <table class="w-full whitespace-no-wrap border">
                    <thead>
                        <tr class="text-left bg-gray-200 text-gray-700">
                            <th class="px-4 py-2 w-1/5 font-semibold">Date</th>
                            <th class="px-4 py-2 w-1/5 font-semibold">Status</th>
                            <th class="px-8 py-2 w-1/5 font-semibold">Title</th>
                            <th class="px-2 py-2 w-1/5 font-semibold">Category</th>
                    ...
                    <tbody class="bg-white divide-y dark:divide-gray-400">
                        @foreach ($articles as $item)
                            <tr class="bg-white text-gray-500">
                                ...
                                <td class="px-8 py-2 text-xs w-1/5">
                                    {{ implode(' ', array_slice(explode(' ', $item->title), 0, 5)) }}{{ count(explode(' ', $item->title)) > 5 ? '...' : '' }}
                                </td>
                                <td class="px-2 py-2 text-xs w-1/5">
                                    {{ $item->category->category . ' | ID - (' . $item->category->id . ')' }} </td>
                                <td class="px-2 py-2 text-xs flex w-1/5">
                                    ...
                                    <button
                                        ...
                                        <svg width="16px" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 16 16">
